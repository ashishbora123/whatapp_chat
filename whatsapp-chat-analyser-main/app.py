...
if uploaded_file is not None:
    ...
    if st.sidebar.button("Show Analysis"):
        ...
        fig, ax = plt.subplots()
        plt.plot(daily_timeline['only_date'], daily_timeline['message'], color = '#92e9b2')
        ...
        fig, ax = plt.subplots()
        plt.plot(monthly_timeline_df['time'], monthly_timeline_df['message'], color = '#92e9b2')
        ax.set_facecolor('#dcf8c6')
        plt.xticks(rotation='vertical')
        st.pyplot(fig)
        ...
        with col1:
            ...
            fig,ax = plt.subplots()
            ax.bar(active_day.index, active_day.values, color = '#92e9b2')
            ax.set_facecolor('#dcf8c6')
            plt.xticks(rotation='vertical')
            st.pyplot(fig)
        ...
        with col2:
            ...
            fig,ax = plt.subplots()
            ax.bar(active_month.index, active_month.values, color = '#92e9b2')
            ax.set_facecolor('#dcf8c6')
            plt.xticks(rotation='vertical')
            st.pyplot(fig)
        ...
        if selected_user == 'Overall':
            ...
            with col1:
                ax.bar(active_user.index, active_user.values, color = '#92e9b2')
                ax.set_facecolor('#dcf8c6')
                plt.xticks(rotation= 'vertical')
                st.pyplot(fig)
            with col2:
        ...
        with col1:
            fig, ax = plt.subplots()
            ax.barh(most_common_words_df[0], most_common_words_df[1], color = '#92e9b2')
            ax.set_facecolor('#dcf8c6')
            plt.xticks(rotation= 'vertical')
            st.pyplot(fig)
